...
const getLayer0ConfigFilePath = require('./getLayer0ConfigFilePath');
...
module.exports = async function init(args) {
  ...
  } // Install the Layer0 CLI globally if it doesn't exist
  ...
  console.log(cyan('    0 dev\n'));
  logger.title(`To ${deploy ? 're' : ''}deploy your app:\n`);
  console.log(cyan('    0 deploy\n'));
};
/**
...
async function ensureNodePackage(args) {
  ...
  if (isNodePackageRoot()) {
    ...
    const choices = await prompts([{
      ...
      choices: [{
        title: `Add Layer0 to the current app${nodePackageName ? ` (${nodePackageName})` : ''}`,
        value: false
      }, {
...
/**
 * Initializes a new Layer0 app in the current working directory
 * @param {*} args
 */
...
/**
 * Installs all Layer0 build and runtime dependencies
 */
...
async function installAllDependencies({
  ...
  // add @layer0/* as build time dependencies
  const devDependencies = {
    '@layer0/core': layer0Version,
    '@layer0/cli': layer0Version,
    '@layer0/prefetch': layer0Version,
    '@layer0/devtools': layer0Version
  };
  const dependencies = {};
  ...
  if (connector) {
    ...
    const addLib = (lib, depArray) => {
      depArray[lib] = lib.startsWith('@layer0/') ? layer0Version : 'latest';
    ...
    if (!connector.builder.startsWith('./') && !isPackageInstalled(connector.builder)) {
      devDependencies[connector.builder] = layer0Version;
    }
  }
...
/**
 * Runs the connector's init script, or the init script in @layer0/core if the connector
...
/**
 * Updates layer0.config.js based on:
 *
 * - name
 * - team
 * - origin
...
async function updateConfig({
  ...
  const configPath = getLayer0ConfigFilePath();
