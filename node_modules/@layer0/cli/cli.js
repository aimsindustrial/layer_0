...
yargs.scriptName('0').middleware(initContext).commandDir('commands').option('api-url', {
  type: 'string',
  description: 'Layer0 Developer Console URL',
  default: 'https://app.layer0.co'
}).options('token', {
  type: 'string',
  description: 'Authenticate with a specific site deploy token. You can also specify the token by setting the LAYER0_DEPLOY_TOKEN environment variable.'
}).option('verbose', {
  alias: 'v',
  type: 'boolean',
  ...
  default: isCI
}).strict().completion('completion', 'Generates a script that you can add to your shell to enable autocompletions for the layer0 command').demandCommand().middleware([latestVersionWarning]).fail((msg, err, yargs) => {
  try {
    if (err) {
      if (err.isUserError) {
